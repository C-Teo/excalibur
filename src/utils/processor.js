export function processArrayOfArrays(arr) {
  ...
  const requiredOrder = ['Task', 'Task Description', 'Task Due Date', 'Reminder Type', 'Assignee', 'Done?'];
  ...
  const processedArray = rearrangedArray.filter((row, index) => {
    ...
    if (reminderTypeValue !== 'short' && reminderTypeValue !== 'mid' && reminderTypeValue !== 'long') {
      return false;
    }
